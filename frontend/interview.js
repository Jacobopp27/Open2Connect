...
window.InterviewUI = (() => {
  ...
  function reset() {
    speech?.dispose(); session = null; answer = ''; manual = null; error = ''; mcpToken = '';
  }
  ...
  function fields() {
    ...
    return ['name','role','experience','sector','interests','languages','purpose','needs_status','problem','help','priority','outcome','offers_status','skills','knowledge','services','resources','availability','contact'].map(k => {
      ...
      return `<label>${context.esc(name(k))}${choices?`<select data-iv-field="${k}">${choices.map(v=>`<option value="${v}" ${d[k]===v?'selected':''}>${context.esc(label(v))}</option>`).join('')}</select>`:`<input data-iv-field="${k}" maxlength="1500" value="${context.esc(d[k]||'')}">`}</label>`;
    }).join('');
  }
  function readFields() {
  ...
  async function sendAnswer() {
    ...
    try {
      ...
      if (readAloud) speech.speak(session.question,session.locale);
  ...
  document.addEventListener('click', async e => {
    ...
    if(a==='review') readFields();
    pending=true;error='';
    try {
      if(a==='start'){install(await context.api('interviews/start?event='+encodeURIComponent(context.state.event),{mode,locale:document.documentElement.lang,ai_consent:consent}));if(readAloud)speech.speak(session.question,session.locale);}
      else if(a==='new'){reset();}
      ...
      else if(a==='review'){speech.pause();install(await action('summary',{profile:manual}));}
      else if(a==='confirm'){
  ...
  return {init,view,reset,leave:()=>speech?.pause()};
})();
